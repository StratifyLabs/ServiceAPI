...
int Project::publish_build(const ProjectOptions &options) {
  ...
  if (
    (permissions != "public") && (permissions != "private")
  ...
  if ((permissions == "private") && get_team_id().is_empty()) {
    set_error_message(
      "`private` permissions can only be used with a valid team id");
    return -1;
  }
  ...
  // id
  if ((project_document_id == "<invalid>") || project_document_id.is_empty()) {
  ...
  if (
    existing_project.download(
      ProjectOptions().set_document_id(project_document_id))
    < 0) {

    set_error_message(
      "Failed to download existing project `" + project_document_id + "`");
    return -1;
  }
  ...
  CLOUD_PRINTER_TRACE(
    "existing team project id `" + existing_project.get_team_id() + "`");
  CLOUD_PRINTER_TRACE(
    "does user own project " + existing_project.get_user_id()
    + " == " + cloud().credentials().get_uid());
  ...
  if (
    build.import_from_compiled(
      BuildImportOptions().set_path(options.file_path()))
    < 0) {
  ...
  if (readme.open(readme_path, fs::OpenMode::read_only()) >= 0) {

    printer().key("readme", readme_path);
    CLOUD_PRINTER_TRACE("adding README file " + readme_path);

    fs::DataFile readme_datafile(fs::OpenMode::append_read_write());
    readme_datafile.write(readme, fs::File::PageSize(512));
  ...
  build_id = build.upload(
    BuildOptions(project_document_id).set_dry_run(options.is_dry_run()));
  ...
  project_build_list.push_back(
    ProjectBuildItem(build_id).set_version(version.string()));

  set_build_list(project_build_list);

  CLOUD_PRINTER_TRACE(
    "publish build changes to project " + project_document_id);

  if (!options.is_dry_run()) {
    bool upload_result = upload(project_document_id).is_empty();

    if (upload_result == true) {
      CLOUD_PRINTER_TRACE("failed with message " + error_message());

      set_error_message("failed to update project");
...
var::String Project::get_storage_path(const PublishBuild &options) const {
  ...
  String result;
  result = "builds/" + get_document_id() + "/" + build_id + "/"
...
Build Project::download_build(const var::String &version) const {
  ...
  result.download(BuildOptions(get_document_id()).set_document_id(build_id));
